...
Canvas::Canvas(size_t w, size_t h) : w(w), h(h) {
  ...
  render =
      std::vector<std::vector<std::string>>(h - 2, std::vector<std::string>(w));
...
void Canvas::getClusterCenters() {
  ...
  for (int i = 0; i < clusterCount; i++) {
    clusterCenters[i] = std::pair<int, int>{
        round((rand() % (int)round(h - (2 * (h * RenderConfig::VIGNETTE)))) +
              (h * RenderConfig::VIGNETTE)),
        round((rand() % (int)round(w - (2 * (w * RenderConfig::VIGNETTE)))) +
              (w * RenderConfig::VIGNETTE)),
    };
  }
}
...
void Canvas::spawnStars() {
  for (int i = 0; i < starfield.size(); i++) {
    for (int j = RenderConfig::PADDING;
         j < starfield[i].size() - RenderConfig::PADDING; j++) {
      int minDist = GetManhattanDist(i, j, clusterCenters[0].first,
                                     clusterCenters[0].second);
      int clusterIndex = 0;

      for (int c = 1; c < clusterCenters.size(); c++) {
        int dist = GetManhattanDist(i, j, clusterCenters[c].first,
                                    clusterCenters[c].second);
        if (dist < minDist) {
          minDist = dist;
          clusterIndex = c;
        }
      ...
      if (rand() % minDist == 0) {
        ...
        for (int x = 1; x <= RenderConfig::HORIZ_PLACEMENT_PADDING; x++)
          if (starfield[i][std::max(0, j - x)] != ' ' ||
              starfield[i][std::min((int)starfield[i].size() - 1, j + x)] !=
                  ' ')
            crowded = true;
        for (int x = 1; x <= RenderConfig::VERT_PLACEMENT_PADDING; x++)
          if (starfield[std::max(0, i - x)][j] != ' ' ||
              starfield[std::min((int)starfield.size() - 1, i + x)][j] != ' ')
            crowded = true;
...
void Canvas::renderStars() {
  ...
  std::vector<std::vector<std::string>> colorBands = {{"\x1b[31m", "\x1b[33m"},
                                                      {"\x1b[34m", "\x1b[35m"},
                                                      {"\x1b[36m", "\x1b[32m"}};
  // std::vector<std::vector<std::string>> colorBands = {{COLORS[0], COLORS[1]},
  // {COLORS[2], COLORS[3]}, {COLORS[4], COLORS[5]}};
  // std::vector<std::vector<std::string>> colorBands = {{COLORS[0]},
  ...
  for (int i = 0; i < starfield.size(); i++) {
    for (int j = 0; j < starfield[i].size(); j++) {
      ...
      // if we're in the right band, have a 50% chance of picking that band's
      // color bands are calculated radially using manhattan distance
      ...
      // Place color and primary star char on canvas, but avoid overwriting
      // existing chars (can't do direct std::string comparison to check because
      // colors are embedded in the std::strings)
      bool overWrite = true;
  ...
  for (int i = 0; i < maxShipCount; i++) {
    if (rand() % probability == 0) {
      std::vector<std::vector<char>> ship =
          LiteralToMat(SHIP_ART[rand() % SHIP_ART.size()]);
